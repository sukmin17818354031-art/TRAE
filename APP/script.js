...
let nextTaskNameValue = ''; // 下一轮任务名称
let nextDurationValue = 25; // 下一轮专注时长（分钟）
let isEditingNextTask = false; // 是否正在编辑下一轮任务
...
function bindEventListeners() {
  ...
  // 下一轮任务名称输入
  nextTaskName.addEventListener('input', () => {
    // 仅在休息时间允许编辑
    if (!isBreakTime) {
      alert('只能在休息时间编辑下一轮任务信息');
      nextTaskName.value = nextTaskNameValue;
    }
  });
  
  // 下一轮任务时长输入
  nextDurationInput.addEventListener('input', () => {
    // 仅在休息时间允许编辑
    if (!isBreakTime) {
      alert('只能在休息时间编辑下一轮任务信息');
      nextDurationInput.value = nextDurationValue;
    }
  });
...
function startTimer() {
  ...
  // 无论是否是休息时间，只要点击开始按钮且未运行，就开始倒计时
  if (!isRunning) {
    isRunning = true;
    startBtn.disabled = true;
    pauseBtn.disabled = false;
    
    console.log('开始倒计时，isBreakTime:', isBreakTime, 'currentTime:', currentTime);
    
    timerInterval = setInterval(() => {
      if (currentTime > 0) {
        currentTime--;
        updateTimerDisplay();
        console.log('倒计时中，剩余时间:', currentTime);
      } else {
        clearInterval(timerInterval);
        isRunning = false;
        console.log('倒计时结束，isBreakTime:', isBreakTime);
        
        if (isBreakTime) {
          // 休息时间结束
          isBreakTime = false;
          console.log('休息时间结束，准备进入下一轮专注');
          
          // 隐藏下一轮任务编辑界面
          nextTaskInfo.style.display = 'none';
          console.log('隐藏下一轮任务编辑界面');
          
          // 获取用户编辑的下一轮任务信息
          const editedNextTaskName = nextTaskName.value.trim();
          const editedNextDuration = parseInt(nextDurationInput.value) || 25;
          console.log('获取编辑的下一轮任务信息:', editedNextTaskName, editedNextDuration);
          
          // 检查是否编辑了下一轮任务名称
          if (editedNextTaskName && editedNextTaskName !== (currentTask ? currentTask.name : '')) {
            // 编辑了任务名称，创建新任务
            console.log('编辑了任务名称，创建新任务:', editedNextTaskName);
            
            // 设置新的任务信息
            const taskTypeValue = taskType.value === '自定义' ? customTaskType.value.trim() || '其他' : taskType.value;
            currentTask = {
              id: Date.now(),
              name: editedNextTaskName,
              type: taskTypeValue,
              duration: editedNextDuration,
              status: '进行中',
              createdAt: new Date().toISOString()
            };
            
            // 更新时长
            originalTime = editedNextDuration * 60;
            currentTime = originalTime;
            updateTimerDisplay();
            
            // 自动开始下一轮专注倒计时
            startTimer();
          } else if (editedNextDuration !== (originalTime / 60)) {
            // 未编辑任务名称，但修改了时长，继续使用当前任务名称，累积时间
            console.log('未编辑任务名称，但修改了时长，累积时间');
            ...
            // 未编辑任务名称和时长，继续使用当前设置
            console.log('未编辑任务名称和时长，继续使用当前设置');
          ...
          // 延迟1.5秒后开始休息时间，让撒花动画有足够的时间显示
          console.log('准备开始休息时间，时长:', breakDuration);
          setTimeout(() => {
            startBreakTime();
          }, 1500);
        }
...
function startBreakTime() {
  ...
  updateTimerDisplay();
  console.log('进入休息时间，时长:', breakDuration, 'isBreakTime:', isBreakTime);
  
  // 保存当前任务信息作为下一轮的默认值
  if (currentTask) {
    nextTaskNameValue = currentTask.name;
    nextDurationValue = originalTime / 60;
    console.log('保存下一轮默认任务:', nextTaskNameValue, '默认时长:', nextDurationValue);
  ...
  console.log('显示下一轮任务编辑界面');
  
  // 直接调用startTimer函数开始休息倒计时
  console.log('直接调用startTimer函数');
  startTimer();
}
